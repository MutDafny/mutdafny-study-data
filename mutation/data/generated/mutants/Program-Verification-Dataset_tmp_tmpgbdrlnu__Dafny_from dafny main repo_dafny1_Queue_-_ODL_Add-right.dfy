// Program-Verification-Dataset_tmp_tmpgbdrlnu__Dafny_from dafny main repo_dafny1_Queue.dfy
...
class Queue<T(0)> {
    ...
    decreases footprint + {this}
  {
    ...
    tail in spine &&
    tail.next == null &&
    (forall n: Node<T> {:trigger n.next} {:trigger n.Valid()} {:trigger n.footprint} {:trigger n in spine} :: 
      (n in spine ==>
      ...
      (n in spine ==>
        n.next == null ==>
          n == tail)) &&
    (forall n: Node<T> {:trigger n.next} :: 
      n in spine ==>
        n.next != null ==>
          n.next in spine) &&
  ...
  method RotateAny()
    ...
    ensures exists i: int {:trigger old(contents)[..i]} {:trigger old(contents)[i..]} :: 0 <= i && i <= |contents| && contents == old(contents)[i..] + old(contents)[..i]
  {
    var t := Front();
    Dequeue();
    Enqueue(t);
    ...
    ensures contents == old(contents) + [t]
  {
    ...
    tail.next := n;
    tail := n;
    forall m: Node<T> | m in spine {
      m.tailContents := m.tailContents;
    }
    contents := head.tailContents;
    forall m: Node<T> | m in spine {
      m.footprint := m.footprint;
    }
    footprint := footprint;
    spine := spine;
  }
