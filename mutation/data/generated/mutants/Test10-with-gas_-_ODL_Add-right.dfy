...
module Test10Gas {
    ...
    decreases c, g
  {
    ...
    while vm.Peek(0) > 0
      ...
      decreases vm.Peek(0) as int - 0 as int
    {
      vm := Push1(vm, a).UseGas(G_VERYLOW);
      vm := Push1(vm, b).UseGas(G_VERYLOW);
      vm := Add(vm).UseGas(G_VERYLOW);
      vm := Pop(vm).UseGas(G_BASE);
      vm := Push1(vm, 1).UseGas(G_VERYLOW);
    ...
    decreases c, g
  {
    ...
    vm := Push1(vm, 0).UseGas(G_VERYLOW);
    vm := Push1(vm, c).UseGas(G_VERYLOW);
    while vm.Peek(0) > 0
      invariant vm.EXECUTING?
      ...
      decreases c - count
    {
      vm := Push1(vm, 2).UseGas(G_VERYLOW);
      vm := Dup(vm, 3).UseGas(G_VERYLOW);
      vm := Add(vm).UseGas(G_VERYLOW);
      vm := Swap(vm, 2).UseGas(G_VERYLOW);
      vm := Pop(vm).UseGas(G_BASE);
      vm := Push1(vm, 1).UseGas(G_VERYLOW);
      vm := Swap(vm, 1).UseGas(G_VERYLOW);
      vm := Sub(vm).UseGas(G_VERYLOW);
