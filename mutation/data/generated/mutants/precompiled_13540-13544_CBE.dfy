...
module Precompiled {
    ...
    decreases data
  {
    ...
    if x0 == None || y0 == None then
      ...
      var p0: Option<AltBn128.Point> := BNP(x0.Unwrap(), y0.Unwrap()); if p0 == None then None else var p: Point := p0.Unwrap().Mul(n as nat); var (r_x: u256, r_y: u256) := (0, 0); var bytes: Array<u8> := U256.ToBytes(r_x) + U256.ToBytes(r_y); assert |bytes| == 64; Some((bytes, G_BNMUL))
  }
