...
module ConcreteEval {
    ...
    decreases fuel
  {
    if fuel == 0 then
      ...
      match stmt_step(env, ss[pc]) { case None() => Error case Some((env', offset)) => if !(0 <= pc + offset <= |ss|) then Error else stmts_step(env', ss, pc + offset, fuel) }
  }
    ...
    decreases env, e
  {
    match e {
      ...
      case Sub(r1, r2) =>
        var v1: Val := env.rs(r1);
        var v2: Val := env.rs(r2);
        Interval(0, if v1.hi >= 0 then v1.hi else 0)
    }
  }
