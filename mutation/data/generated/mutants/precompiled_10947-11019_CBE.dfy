...
module Precompiled {
    ...
    decreases data
  {
    ...
    if x0 == None || y0 == None || x1 == None || y1 == None then
      ...
      var p0: Option<AltBn128.Point> := BNP(x0.Unwrap(), y0.Unwrap()); var p1: Option<AltBn128.Point> := BNP(x1.Unwrap(), y1.Unwrap()); if p0 == None || p1 == None then None else var p: Point := p0.Unwrap().Add(p1.Unwrap()); var (r_x: u256, r_y: u256) := var Pair(p_x: Element, p_y: Element) := p; (p_x.n as u256, p_y.n as u256); var bytes: Array<u8> := U256.ToBytes(r_x) + U256.ToBytes(r_y); assert |bytes| == 64; Some((bytes, G_BNADD))
  }
