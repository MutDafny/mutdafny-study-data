  ...
  class Set {
      ...
      decreases stop - start
    {
      if start == stop then
        ...
        if s1[start] in s2 then intersect_length(s1, s2, count, start, stop) else intersect_length(s1, s2, count, start, stop)
    }
      ...
      decreases stop - i
    {
      if i == stop then
        ...
        if s1[i] !in s2 then union_length(s1, s2, count, i, stop) else union_length(s1, s2, count, i, stop)
    }
