    ...
    decreases st
  {
    if st.Operands() >= 1 then
      if !st.WritesPermitted() then
        ...
        var address: u160 := st.evm.context.address; var balance: u256 := st.evm.world.Balance(address); var r: u160 := (st.Peek(0) as nat % TWO_160) as u160; var ss: Raw := st.evm.substate.AccountAccessed(r).AccountDestructed(address); var w: T := st.evm.world.Withdraw(address, balance); RETURNS(gas := st.Gas(), data := [], world := w, transient := st.evm.transient, substate := ss)
    else
      ERROR(STACK_UNDERFLOW)
  }
