    ...
    decreases st, offset
  {
    if st.Operands() >= 1 then
      ...
      if val == 0 then
        ...
        var post_pc: int := (st.evm.pc + 3) as int; var target: int := post_pc + offset as int; if st.IsInstructionBoundary(0) then st.Pop(1).Goto(target as u256) else ERROR(INVALID_JUMPDEST)
    else
      ERROR(STACK_UNDERFLOW)
  }
