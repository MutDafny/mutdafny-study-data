...
module Precompiled {
  ...
  {
    -if lE <= 32 then var E_bytes: seq<u8> := Arrays.SliceAndPad(data, 96 + lB, lE, 0); var w: u256 := ByteUtils.ReadUint256(ByteUtils.LeftPad(E_bytes, 32), 0); if w == 0 then 0 else U256.Log2(w) else var w: u256 := ByteUtils.ReadUint256(data, 96 + lB); var g: int := 8 * (lE - 32); if 32 < lE && w != 0 then g + U256.Log2(w) else g
  }
