    ...
    decreases st
  {
    if st.Operands() >= 7 then
      ...
      if !st.WritesPermitted() && value != 0 then
        ...
        var calldata: seq<u8> := Memory.Slice(st.evm.memory, inOffset, inSize); var address: u160 := st.evm.context.address; var nst: ExecutingState := st.AccountAccessed(to).UseGas(gascap).Expand(inOffset, inSize).Expand(outSize, outOffset).Pop(7).Next(); CONTINUING(CALLS(nst.evm, address, to, to, callgas, value, value, calldata, st.evm.context.writePermission, outOffset := outOffset, outSize := outSize))
    else
      ERROR(STACK_UNDERFLOW)
  }
