...
module Gas {
  ...
  function CostSStore(st: ExecutingState): nat
    decreases st
  {
    -var currentAccount: Account := st.evm.world.GetAccount(st.evm.context.address).Unwrap(); var originalAccount: Account := st.evm.world.GetOrDefaultPretransaction(st.evm.context.address); if st.Gas() <= G_CALLSTIPEND then MAX_U256 else if st.Operands() >= 2 then var loc: u256 := st.Peek(0); var newVal: nat := st.Peek(1) as nat; var accessCost: int := if st.WasKeyAccessed(loc) then 0 else G_COLDSLOAD; var currentVal: nat := Storage.Read(currentAccount.storage, loc) as nat; var originalVal: nat := Storage.Read(originalAccount.storage, loc) as nat; CostSStoreCalc(originalVal, currentVal, newVal) + accessCost else G_ZERO
  }
