...
method quickSort(intSeq: array<int>)
  modifies intSeq
  ensures forall i: nat, j: nat {:trigger intSeq[j], intSeq[i]} | 0 <= i <= j < intSeq.Length :: intSeq[i] <= intSeq[j]
  decreases intSeq

lemma sort(prevSeq: seq<int>) returns (curSeq: seq<int>)
  ensures forall i: nat, j: nat {:trigger curSeq[j], curSeq[i]} | 0 <= i <= j < |curSeq| :: curSeq[i] <= curSeq[j]
  ensures multiset(prevSeq) == multiset(curSeq)
  decreases prevSeq

predicate post_sort(prevSeq: seq<int>, curSeq: seq<int>)
  decreases prevSeq, curSeq
{
  (forall i: nat, j: nat {:trigger curSeq[j], curSeq[i]} | 0 <= i <= j < |curSeq| :: curSeq[i] <= curSeq[j]) <==> multiset(prevSeq) == multiset(curSeq)
}
...
lemma twoSortedSequencesWithSameElementsAreEqual(s1: seq<int>, s2: seq<int>)
  requires forall i: nat, j: nat {:trigger s1[j], s1[i]} | 0 <= i <= j < |s1| :: s1[i] <= s1[j]
  requires forall i: nat, j: nat {:trigger s2[j], s2[i]} | 0 <= i <= j < |s2| :: s2[i] <= s2[j]
...
lemma sort_determinisitc(prevSeq: seq<int>, curSeq: seq<int>, curSeq': seq<int>)
  ...
  decreases prevSeq, curSeq, curSeq'
{
  if |curSeq| != |curSeq'| {
