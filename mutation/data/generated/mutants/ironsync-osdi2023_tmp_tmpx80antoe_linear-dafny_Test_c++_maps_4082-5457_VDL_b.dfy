...
method TestNullsAmongValues()
{
  ...
  var d := new MyClass("jack");
  var e := new MyClass("sibylla");
  var m: map<uint32, MyClass?> := map[0 := a, 2 := null, 3 := null];
  var n: map<uint32, MyClass?> := map[0 := d, 20 := null, 4 := e];
  var o: map<uint32, MyClass?> := map[199 := null, 198 := a];
  var o': map<uint32, MyClass?> := map[55 := null, 198 := a];
  var o'': map<uint32, MyClass?> := map[56 := null, 198 := a];
