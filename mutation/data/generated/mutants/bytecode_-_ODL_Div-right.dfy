...
module Gas {
    ...
    decreases mem, address, len
  {
    if len == 0 || address + len - 1 < |mem.contents| then
      ...
      var before: int := |mem.contents|; var after: nat := Memory.SmallestLarg32(address + len - 1); QuadraticCostIsMonotonic(after, before); assert QuadraticCost(after) >= QuadraticCost(before); QuadraticCost(after) - QuadraticCost(before)
  }
    ...
    decreases fork, len
  {
    if fork.IsActive(3860) then
      G_INITCODE_WORD_COST * Int.RoundUp(len, 32)
    else
      0
  }
...
module U256 {
    ...
    decreases lhs, rhs
  {
    if rhs >= 256 then
      ...
      var p: nat := MathUtils.Pow(2, rhs as nat); var n: nat := lhs as nat; n as u256
  }
  ...
  function NthUint8(v: u256, k: nat): u8
    ...
    decreases v, k
  {
    var w128: u128 := NthUint128(v, k);
    var w64: u64 := U128.NthUint64(w128, k % 16);
    var w32: u32 := U64.NthUint32(w64, k % 8);
    var w16: u16 := U32.NthUint16(w32, k % 4);
...
module MathUtils {
    ...
    decreases k
  {
    ...
    if k == 0 || m == 1 then
      ...
      var np: nat := ModPow(nm, k, m); var np2: int := np * np % m; if k % 2 == 0 then np2 else np2 * nm % m
  }
...
module Precompiled {
    ...
    decreases data
  {
    ...
    var gascost: int := Int.Max(200, f(Int.Max(lM, lB)) * Int.Max(lEp, 1));
    Some((output, gascost))
  }
