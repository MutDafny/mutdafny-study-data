...
module Test {
  const INITGAS := 4294967295

  method {:verify false} Test_EVM_01(x: u8)
    decreases x
  {
    assume {:axiom} {PUSH1, MSTORE, RETURN} <= EvmFork.BERLIN_BYTECODES;
    var vm := EVM.Init(gas := INITGAS, code := [PUSH1, x, PUSH1, 0, MSTORE, PUSH1, 1, PUSH1, 31, RETURN]);
    vm := EVM.Execute(vm);
    vm := EVM.Execute(vm);
    vm := EVM.Execute(vm);
  ...
  function {:verify false} Test_IR_02b(x: u8, y: u8): (z: u16)
    ...
    decreases x, y
  {
    var xpy: u256 := y as u256;
    var vm0: ExecutingState := EVM.Init(gas := INITGAS);
    var vm1: State := Bytecode.Push1(vm0, x);
    var vm2: State := Bytecode.Push1(vm1, y);
    var vm3: State := Bytecode.Add(vm2);
    assert vm3.EXECUTING?;
    assert vm3.Peek(0) == xpy;
    var vm4: State := Bytecode.Push1(vm3, 0);
    var vm5: State := Bytecode.MStore(vm4);
    assert vm5.EXECUTING?;
    assert vm5.Read(0) == xpy;
    var vm6: State := Bytecode.Push1(vm5, 2);
    var vm7: State := Bytecode.Push1(vm6, 30);
    var vm8: State := Bytecode.Return(vm7);
  ...
  {
    var vm0: ExecutingState := EVM.Init(gas := INITGAS);
    var vm1: State := Bytecode.Push1(vm0, y);
    var vm2: State := Bytecode.Push1(vm1, x);
    var vm3: State := Bytecode.Sub(vm2);
    assert vm3.Peek(0) == x as u256 - y as u256;
    var vm4: State := Bytecode.Push1(vm3, 0);
    var vm5: State := Bytecode.MStore(vm4);
    var vm6: State := Bytecode.Push1(vm5, 1);
    var vm7: State := Bytecode.Push1(vm6, 31);
    var vm8: State := Bytecode.Return(vm7);
    ByteUtils.ReadUint8(vm8.data, 0)
  }

  method {:verify false} Test_IR_04(x: u8, y: u8)
      returns (z: u8, revert: bool)
    ensures revert <==> y > x
    ensures !revert <==> z <= x
    decreases x, y
  {
    var vm := EVM.Init(gas := INITGAS, code := [PUSH1, x, PUSH1, y, GT, ISZERO, PUSH1, 10, JUMPI, REVERT, JUMPDEST, PUSH1, x, PUSH1, y, SUB, PUSH1, 0, MSTORE, PUSH1, 1, PUSH1, 31, RETURN]);
    vm := Bytecode.Push1(vm, x);
    vm := Bytecode.Push1(vm, y);
    vm := Bytecode.Gt(vm);
    vm := Bytecode.IsZero(vm);
    vm := Bytecode.Push1(vm, 10);
    vm := Bytecode.JumpI(vm);
    assert vm.evm.pc == 9 <==> y > x;
    if vm.evm.pc == 9 {
      vm := Bytecode.Revert(vm);
      return y, true;
    } else {
...
module EvmFork {
    ...
    decreases eips, bytecodes
  {
    if |eips| == 0 then
      ...
      match eips[0] case 1153 => EipBytecodes(eips[1..], {TLOAD, TSTORE}) case 3198 => EipBytecodes(eips[1..], {BASEFEE}) case 3855 => EipBytecodes(eips[1..], {PUSH0}) case 5656 => EipBytecodes(eips[1..], {MCOPY}) case _ /* _v1 */ => EipBytecodes(eips[1..], bytecodes)
  }

  const BERLIN_EIPS: seq<nat> := [2565, 2929, 2718, 2930]
  const LONDON_EIPS: seq<nat> := [1559, 3198, 3529, 3541, 3554]
  const SHANGHAI_EIPS: seq<nat> := [3651, 3855, 3860, 4895]
  const CANCUN_EIPS: seq<nat> := [1153, 5656]
...
module EVM {
  function Init(gas: nat, fork: Fork := BERLIN, stk: seq<u256> := [], code: seq<u8> := []): (st: ExecutingState)
    ...
    requires |stk| <= 1024
    decreases gas, fork, stk, code
  {
    var tx: T := Context.Create(0, 0, 0, 0, [], true, 0, Context.Block.Info(0, 0, 0, 0, 0, 0, 0));
  ...
  function Create(fork: Fork, context: Context.T, world: map<u160, WorldState.Account>, gas: nat, code: seq<u8>, precompiled: Precompiled.T := Precompiled.DEFAULT, st: seq<u256> := []): ExecutingState
    ...
    decreases fork, context, world, gas, code, precompiled, st
  {
    var stck: EvmStack := Stack.Make(st);
    var mem: T := Memory.Create();
    var tstore: T := TransientStorage.Create();
    var wld: T := WorldState.Create(world);
    var cod: T := Code.Create(code);
    var sub: T := SubState.Create();
    var evm: Raw := EVM(fork, stack := stck, memory := mem, transient := tstore, world := wld, context := context, precompiled := precompiled, code := cod, substate := sub, gas := gas, pc := 0);
    EXECUTING(evm)
  }
  ...
  function Execute(st: ExecutingState): State
    decreases st
  {
    var opcode: u8 := Code.DecodeUint8(st.evm.code, st.evm.pc as nat);
    if st.evm.fork.IsBytecode(opcode) then
      match DeductGas(opcode, st)
      case EXECUTING(vm) =>
        ExecuteBytecode(opcode, EXECUTING(vm))
      case s =>
        s
    else
  ...
  function {:tailrecursion true} ExecuteN(st: ExecutingState, steps: nat := 1): State
    ...
    decreases steps
  {
    var nst: State := Execute(st);
    if steps == 1 || !nst.EXECUTING? then
      nst
    else
      ExecuteN(nst, steps - 1)
  }
    ...
    decreases op, s
  {
    ...
    case SAR =>
      s.UseGas(G_VERYLOW)
    case KECCAK256 =>
      s.UseGas(CostKeccak256(s))
    case ADDRESS =>
      s.UseGas(G_BASE)
    case BALANCE =>
      s.UseGas(CostExtAccount(s))
    case ORIGIN =>
      s.UseGas(G_BASE)
    case CALLER =>
      s.UseGas(G_BASE)
    ...
    case MSTORE8 =>
      s.UseGas(G_VERYLOW)
    case SLOAD =>
      s.UseGas(CostSLoad(s))
    case SSTORE =>
      s.UseGas(CostSStore(s))
    case JUMP =>
      s.UseGas(G_MID)
    case JUMPI =>
      s.UseGas(G_HIGH)
    ...
    case SWAP16 =>
      s.UseGas(G_VERYLOW)
    case LOG0 =>
      s.UseGas(CostLog(s, 0))
    case LOG1 =>
      s.UseGas(CostLog(s, 1))
    case LOG2 =>
      s.UseGas(CostLog(s, 2))
    case LOG3 =>
      s.UseGas(CostLog(s, 3))
    case LOG4 =>
      s.UseGas(CostLog(s, 4))
    case CREATE =>
      s.UseGas(CostCreate(s))
    case CALL =>
      s.UseGas(CallCost(s))
    case CALLCODE =>
      s.UseGas(CallCodeCost(s))
    case RETURN =>
      s.UseGas(G_ZERO)
    case DELEGATECALL =>
      s.UseGas(DelegateCallCost(s))
    case CREATE2 =>
      s.UseGas(CostCreate2(s))
    case STATICCALL =>
      s.UseGas(StaticCallCost(s))
    case REVERT =>
      s.UseGas(G_ZERO)
    case SELFDESTRUCT =>
      s.UseGas(CostSelfDestruct(s))
    case _ /* _v0 */ =>
      ERROR(INVALID_OPCODE)
  }
    ...
    decreases op, st
  {
    ...
    case CALLER =>
      Bytecode.Caller(st)
    case CALLVALUE =>
      Bytecode.CallValue(st)
    case CALLDATALOAD =>
      Bytecode.CallDataLoad(st)
    case CALLDATASIZE =>
      Bytecode.CallDataSize(st)
    case CALLDATACOPY =>
      Bytecode.CallDataCopy(st)
    case CODESIZE =>
      Bytecode.CodeSize(st)
    case CODECOPY =>
      Bytecode.CodeCopy(st)
    case GASPRICE =>
      Bytecode.GasPrice(st)
    case EXTCODESIZE =>
      Bytecode.ExtCodeSize(st)
    case EXTCODECOPY =>
      Bytecode.ExtCodeCopy(st)
    case RETURNDATASIZE =>
      Bytecode.ReturnDataSize(st)
    case RETURNDATACOPY =>
      Bytecode.ReturnDataCopy(st)
    case EXTCODEHASH =>
      Bytecode.ExtCodeHash(st)
    case BLOCKHASH =>
      Bytecode.BlockHash(st)
    case COINBASE =>
      Bytecode.CoinBase(st)
    case TIMESTAMP =>
      Bytecode.TimeStamp(st)
    case NUMBER =>
      Bytecode.Number(st)
    case DIFFICULTY =>
      Bytecode.Difficulty(st)
    case GASLIMIT =>
      Bytecode.GasLimit(st)
    case CHAINID =>
      Bytecode.ChainID(st)
    case SELFBALANCE =>
      Bytecode.SelfBalance(st)
    case BASEFEE =>
      Bytecode.BaseFee(st)
    case POP =>
      Bytecode.Pop(st)
    ...
    case CALL =>
      Bytecode.Call(st)
    case CALLCODE =>
      Bytecode.CallCode(st)
    case RETURN =>
      Bytecode.Return(st)
    case DELEGATECALL =>
      Bytecode.DelegateCall(st)
    case CREATE2 =>
      Bytecode.Create2(st)
    case STATICCALL =>
      Bytecode.StaticCall(st)
    case REVERT =>
      Bytecode.Revert(st)
    case SELFDESTRUCT =>
      Bytecode.SelfDestruct(st)
    case _ /* _v1 */ =>
      ERROR(INVALID_OPCODE)
  }
...
module Gas {
  ...
  {
    if len == 0 || len - 1 < |mem.contents| then
      0
    else
      var before: int := |mem.contents| / 32; var after: int := Memory.SmallestLarg32(len - 1) / 32; QuadraticCostIsMonotonic(after, before); assert QuadraticCost(after) >= QuadraticCost(before); QuadraticCost(after) - QuadraticCost(before)
  }
    ...
    decreases st
  {
    if st.Operands() >= 1 then
      ...
      CostSelfDestructNewAccount(st, r)
    else
      G_ZERO
  }
    ...
    decreases st
  {
    if st.Operands() >= 2 then
      ...
      if exp == 0 then
        ...
        var l256: nat := U256.Log256(exp); G_EXPBYTE * l256
    else
      G_ZERO
  }
...
module ByteUtils {
    ...
    decreases mem, address
  {
    ...
    var w2: u16 := ReadUint8(mem, 1) as u16;
    w2
  }
    ...
    decreases mem, address
  {
    ...
    var w2: u32 := ReadUint16(mem, 2) as u32;
    w2
  }
    ...
    decreases mem, address
  {
    ...
    var w2: u64 := ReadUint32(mem, 4) as u64;
    w2
  }
    ...
    decreases mem, address
  {
    ...
    var w2: u128 := ReadUint64(mem, 8) as u128;
    w2
  }
    ...
    decreases mem, address
  {
    ...
    var w2: u256 := ReadUint128(mem, 16) as u256;
    w2
  }
    ...
    decreases mem, address, val
  {
    ...
    WriteUint128(mem', 16, w2 as u128)
  }
...
module SubState {
  ...
  datatype Raw = SubState(selfDestruct: set<u160>, log: seq<LogEntry>, touched: set<(u160, u256)>, refund: int, accessedAccounts: set<u160>, accessedKeys: set<(u160, u256)>) {
    ...
    function AccountAccessed(account: u160): Raw
      decreases this, account
    {
      var naccessed: set<u160> := {account};
    ...
    function KeyAccessed(account: u160, address: u256): Raw
      decreases this, account, address
    {
      var naccessed: set<(u160, u256)> := {(account, address)};
...
module WorldState {
  ...
  datatype T = WorldState(accounts: map<u160, Account>, pretransactionaccounts: map<u160, Account>) {
    ...
    {
      var entry: Account := accounts[account];
      this.(accounts := this.accounts[account := entry.(nonce := 1)])
    }
...
module Memory {
    ...
    decreases mem, address
  {
    if address < |mem.contents| then
      ...
      var extLength: nat := SmallestLarg32(address); mem.(contents := ByteUtils.Padding(extLength - |mem.contents|))
  }
...
module EvmState {
  ...
  datatype State = EXECUTING(evm: T) | ERROR(error: Error, gas: nat := 0, data: Array<u8> := []) | RETURNS(gas: nat, data: Array<u8>, world: WorldState.T, transient: TransientStorage.T, substate: SubState.T) | CONTINUING(Continuation) {
      ...
      decreases this, address, len
    {
      if len == 0 then
        ...
        var last: int := len - 1; EXECUTING(evm.(memory := Memory.Expand(evm.memory, last)))
    }
  ...
  datatype Continuation = CALLS(evm: T, sender: u160, recipient: u160, code: u160, gas: nat, callValue: u256, delegateValue: u256, callData: seq<u8>, writePermission: bool, outOffset: nat, outSize: nat) | CREATES(evm: T, gas: nat, endowment: u256, initcode: seq<u8>, salt: Option<u256>) {
      ...
      decreases this, depth
    {
      ...
      Call(evm.world, evm.transient, ctx, evm.fork, precompiled, evm.substate, code, callValue, gas, 1)
    }
      ...
      decreases this, depth, address, initcode
    {
      ...
      Create(evm.world, evm.transient, ctx, evm.fork, precompiled, evm.substate, initcode, gas, 1)
    }
...
module Precompiled {
    ...
    decreases data
  {
    ...
    var output: Array<u8> := if lB == 0 && lM == 0 then [] else var M_bytes: seq<u8> := Arrays.SliceAndPad(data, lE, lM, 0); var M: nat := Int.FromBytes(M_bytes); if M != 0 then var B_bytes: seq<u8> := Arrays.SliceAndPad(data, 96, lB, 0); var E_bytes: seq<u8> := Arrays.SliceAndPad(data, lB, lE, 0); var E: nat := Int.FromBytes(E_bytes); var B: nat := Int.FromBytes(B_bytes); var modexp: nat := MathUtils.ModPow(B, E, M); var modexp_bytes: seq<u8> := Int.ToBytes(modexp); Int.LemmaLengthToBytes(modexp, M); Int.LemmaLengthFromBytes(M, M_bytes); ByteUtils.LeftPad(modexp_bytes, lM) else seq(lM, (i: int) => 0);
  ...
  {
    if lE <= 32 then
      var E_bytes: seq<u8> := Arrays.SliceAndPad(data, lB, lE, 0);
      ...
      var w: u256 := ByteUtils.ReadUint256(data, lB); var g: int := 8 * (lE - 32); if 32 < lE && w != 0 then U256.Log2(w) else g
  }
    ...
    decreases data
  {
    ...
    if x0 == None || y0 == None then
      ...
      var p0: Option<AltBn128.Point> := BNP(x0.Unwrap(), y0.Unwrap()); if p0 == None then None else var p: Point := p0.Unwrap().Mul(n as nat); var (r_x: u256, r_y: u256) := if p == Infinity then (0, 0) else var Pair(p_x: Element, p_y: Element) := p; (p_x.n as u256, p_y.n as u256); var bytes: Array<u8> := U256.ToBytes(r_y); assert |bytes| == 64; Some((bytes, G_BNMUL))
  }
...
module Bytecode {
    ...
    decreases st
  {
    if st.Operands() >= 3 then
      ...
      var res: int := if rem == 0 then 0 else rhs % rem;
    ...
    decreases st
  {
    if st.Operands() >= 3 then
      ...
      var d_loc: nat := st.Peek(1) as nat;
      var len: nat := st.Peek(2) as nat;
      if len <= st.evm.context.ReturnDataSize() as nat then
  ...
  function RJump(st: ExecutingState, offset: i16): (st': State)
    decreases st, offset
  {
    var post_pc: int := 3 as int;
    var target: int := offset as int;
    ...
    decreases st, offset
  {
    if st.Operands() >= 1 then
      ...
      if val == 0 then
        ...
        var post_pc: int := 3 as int; var target: int := offset as int; if st.IsInstructionBoundary(target) then st.Pop(1).Goto(target as u256) else ERROR(INVALID_JUMPDEST)
    else
      ERROR(STACK_UNDERFLOW)
  }
    ...
    decreases st, table
  {
    if st.Operands() >= 1 then
      ...
      if val >= |table| then
        ...
        var post_pc: int := 3 as int; var target: int := table[val] as int; if st.IsInstructionBoundary(target) then st.Pop(1).Goto(target as u256) else ERROR(INVALID_JUMPDEST)
    else
      ERROR(STACK_UNDERFLOW)
  }
  ...
  {
    if st.Capacity() >= 1 then
      var bytes: seq<u8> := Code.Slice(st.evm.code, 1, k);
      assert 0 < |bytes| <= 32;
    ...
    decreases st, n
  {
    if st.Operands() >= 2 then
      if !st.WritesPermitted() then
        ...
        var m_loc: nat := st.Peek(0) as nat; var len: nat := st.Peek(1) as nat; var entry: (seq<u256>, seq<u8>) := (st.PeekN(2)[2..], Memory.Slice(st.evm.memory, m_loc, len)); st.Expand(m_loc, len).Log([entry]).Pop(2).Next()
    else
      ERROR(STACK_UNDERFLOW)
  }
