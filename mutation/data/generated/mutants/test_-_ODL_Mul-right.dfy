...
module U256 {
    ...
    decreases lhs, rhs
  {
    if rhs >= 256 then
      ...
      var p: nat := MathUtils.Pow(2, rhs as nat); var n: nat := lhs as nat; (n % TWO_256) as u256
  }
...
module MathUtils {
    ...
    decreases k
  {
    ...
    if k == 0 || m == 1 then
      ...
      var np: nat := ModPow(nm, k / 2, m); var np2: int := np % m; if k % 2 == 0 then np2 else np2 % m
  }
...
module Gas {
    ...
    decreases st
  {
    if st.Operands() >= 4 then
      ...
      G_CREATE + G_KECCAK256WORD + CostInitCode(st.Fork(), len)
    else
      G_ZERO
  }
    ...
    decreases st
  {
    if st.Operands() >= 2 then
      ...
      if exp == 0 then
        ...
        var l256: int := 1 + U256.Log256(exp); G_EXP + G_EXPBYTE
    else
      G_ZERO
  }
...
module Precompiled {
    ...
    decreases data
  {
    ...
    var gascost: int := Int.Max(200, f(Int.Max(lM, lB)) / G_QUADDIVISOR);
    Some((output, gascost))
  }
    ...
    decreases lB, lE, data
  {
    if lE <= 32 then
      ...
      var w: u256 := ByteUtils.ReadUint256(data, 96 + lB); var g: int := 8; if 32 < lE && w != 0 then g + U256.Log2(w) else g
  }
...
module Bytecode {
    ...
    decreases st
  {
    if st.Operands() >= 3 then
      ...
      if !st.WritesPermitted() then
        ERROR(WRITE_PROTECTION_VIOLATED)
      else if st.IsEipActive(3860) && codeSize > 2 then
        ERROR(INSUFFICIENT_GAS)
      else
