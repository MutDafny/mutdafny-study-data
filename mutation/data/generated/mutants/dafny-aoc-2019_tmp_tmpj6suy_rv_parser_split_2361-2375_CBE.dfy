...
module Split {
  ...
  function split(s: string, separator: string): seq<string>
    ...
    decreases s, separator
  {
    splitHelper(s, separator, 0, 0, [])
  }

  predicate Contains(haystack: string, needle: string)
    ensures Contains(haystack, needle) <==> exists k: int {:trigger haystack[k..]} :: 0 <= k <= |haystack| && needle <= haystack[k..]
    ensures Contains(haystack, needle) <==> exists i: int {:trigger haystack[i..]} :: 0 <= i <= |haystack| && needle <= haystack[i..]
    ensures !Contains(haystack, needle) <==> forall i: int {:trigger haystack[i..]} :: 0 <= i <= |haystack| ==> !(needle <= haystack[i..])
    decreases haystack, needle
  {
    if needle <= haystack then
      ...
      assert forall i: int {:trigger haystack[i..]} :: 1 <= i <= |haystack| ==> haystack[i..] == haystack[1..][i - 1..];
