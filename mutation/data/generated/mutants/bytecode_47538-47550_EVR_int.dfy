    ...
    decreases st, table
  {
    if st.Operands() >= 1 then
      ...
      if val >= |table| then
        ...
        var post_pc: int := 0 as int; var target: int := post_pc + table[val] as int; if st.IsInstructionBoundary(target) then st.Pop(1).Goto(target as u256) else ERROR(INVALID_JUMPDEST)
    else
      ERROR(STACK_UNDERFLOW)
  }
