...
module EvmState {
  ...
  const EVM_WITNESS: Raw := EVM(BERLIN, Context.DEFAULT, Precompiled.DEFAULT, WorldState.Create(map[0 := WorldState.DefaultAccount()]), EmptyEvmStack, Memory.Create(), TransientStorage.Create(), Code.Create([]), SubState.Create(), 1, 0)
