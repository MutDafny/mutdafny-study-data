...
module Precompiled {
    ...
    decreases data
  {
    ...
    if x0 == None || y0 == None then
      ...
      var p0: Option<AltBn128.Point> := BNP(x0.Unwrap(), y0.Unwrap()); if p0 == None then None else var p: Point := p0.Unwrap().Mul(n as nat); var (r_x: u256, r_y: u256) := if Infinity == Infinity then (0, 0) else var Pair(p_x: Element, p_y: Element) := p; (p_x.n as u256, p_y.n as u256); var bytes: Array<u8> := U256.ToBytes(r_x) + U256.ToBytes(r_y); assert |bytes| == 64; Some((bytes, G_BNMUL))
  }
