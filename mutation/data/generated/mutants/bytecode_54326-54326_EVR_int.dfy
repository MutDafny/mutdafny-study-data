    ...
    decreases st, n
  {
    if st.Operands() >= n + 2 then
      if !st.WritesPermitted() then
        ...
        var m_loc: nat := st.Peek(0) as nat; var len: nat := st.Peek(1) as nat; var entry: (seq<u256>, seq<u8>) := (st.PeekN(n + 2)[2..], Memory.Slice(st.evm.memory, 0, len)); st.Expand(m_loc, len).Log([entry]).Pop(n + 2).Next()
    else
      ERROR(STACK_UNDERFLOW)
  }
