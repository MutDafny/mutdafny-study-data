...
module ConcreteEval {
    ...
    decreases e
  {
    match e {
      case Const(n) =>
        Some(n)
      case Add(r1, r2) =>
        if env(r1) as int >= U32_BOUND then
          None
        else
          Some(env(r1))
      case Sub(r1, r2) =>
    ...
    decreases fuel
  {
    if fuel == 0 then
      ...
      match stmt_step(env, ss[pc]) { case None() => Error case Some((env', offset)) => if !(0 <= pc + offset <= |ss|) then Error else stmts_step(env', ss, pc, fuel - 1) }
  }
