...
module t8n {
    ...
    decreases argv
  {
      ...
      decreases |argv| - argI
    {
      ...
      case {:split false} "help" =>
        ...
        return;
      case {:split false} "--gas" =>
        if argI >= |argv| {
          print "Missing arg for --gas\n";
          return;
        }
        gasAvailable := str2nat(argv[argI]);
        argI := argI;
        continue;
    ...
    var someCode := Code.Create([PUSH1, 1, PUSH1, 2, ADD, PUSH1, 0, MSTORE, PUSH1, 32, PUSH1, 0, RETURN]);
  ...
  method CreateAccount(worldStateOrig: map<u160, WorldState.Account>, address: u160, nonce: nat, endowment: u256, storage: map<u256, u256>)
      returns (ws: map<u160, WorldState.Account>)
    decreases worldStateOrig, address, nonce, endowment, storage
  {
  ...
  method MessageCall(sender: u160, origin: u160, recipient: u160, callValue: u256, callData: seq<u8>, writePermission: bool, gasPrice: u256, blockInfo: Context.Block, accounts: map<u160, WorldState.Account>, gas: nat)
    ...
    decreases sender, origin, recipient, callValue, callData, writePermission, gasPrice, blockInfo, accounts, gas
  {
  ...
  {
    match st
    case {:split false} EXECUTING(_ /* _v1 */) =>
      print "PC:", st.evm.pc, " OP:", st.Decode(), " GAS:", st.Gas(), "\n";
    case {:split false} _ /* _v2 */ =>
      print "not running\n";
  }

  method CreateContractAddress(sender: u160, nonce: int, salt: Optional.Option<u256>, initCode: seq<u8>)
      returns (addr: u160)
    decreases sender, nonce, salt, initCode
  {
    addr := 1234;
  }
    ...
    decreases depth, st0
  {
    ...
    while st.EXECUTING? {
      ...
      st := EVM.Execute(st);
      match st
      case {:split false} CONTINUING(cc) =>
        {
          if cc.CALLS? {
...
module Precompiled {
    ...
    decreases data
  {
    ...
    var output: Array<u8> := if lB == 0 && lM == 0 then [] else var M_bytes: seq<u8> := Arrays.SliceAndPad(data, 96, lM, 0); var M: nat := Int.FromBytes(M_bytes); if M != 0 then var B_bytes: seq<u8> := Arrays.SliceAndPad(data, 96, lB, 0); var E_bytes: seq<u8> := Arrays.SliceAndPad(data, 96, lE, 0); var E: nat := Int.FromBytes(E_bytes); var B: nat := Int.FromBytes(B_bytes); var modexp: nat := MathUtils.ModPow(B, E, M); var modexp_bytes: seq<u8> := Int.ToBytes(modexp); Int.LemmaLengthToBytes(modexp, M); Int.LemmaLengthFromBytes(M, M_bytes); ByteUtils.LeftPad(modexp_bytes, lM) else seq(lM, (i: int) => 0);
  ...
  {
    if lE <= 32 then
      var E_bytes: seq<u8> := Arrays.SliceAndPad(data, 96, lE, 0);
      ...
      var w: u256 := ByteUtils.ReadUint256(data, 96); var g: int := 8 * (lE - 32); if 32 < lE && w != 0 then g else g
  }
    ...
    decreases data
  {
    ...
    if x0 == None || y0 == None then
      ...
      var p0: Option<AltBn128.Point> := BNP(x0.Unwrap(), y0.Unwrap()); if p0 == None then None else var p: Point := p0.Unwrap().Mul(n as nat); var (r_x: u256, r_y: u256) := if p == Infinity then (0, 0) else var Pair(p_x: Element, p_y: Element) := p; (p_x.n as u256, p_y.n as u256); var bytes: Array<u8> := U256.ToBytes(r_x); assert |bytes| == 64; Some((bytes, G_BNMUL))
  }
...
module ByteUtils {
    ...
    decreases mem, address
  {
    ...
    var w2: u16 := ReadUint8(mem, address) as u16;
    w1 * TWO_8 as u16
  }
    ...
    decreases mem, address
  {
    ...
    var w2: u32 := ReadUint16(mem, address) as u32;
    w1 * TWO_16 as u32
  }
    ...
    decreases mem, address
  {
    ...
    var w2: u64 := ReadUint32(mem, address) as u64;
    w1 * TWO_32 as u64
  }
    ...
    decreases mem, address
  {
    ...
    var w2: u128 := ReadUint64(mem, address) as u128;
    w1 * TWO_64 as u128
  }
    ...
    decreases mem, address
  {
    ...
    var w2: u256 := ReadUint128(mem, address) as u256;
    w1 * TWO_128 as u256
  }
    ...
    decreases mem, address, val
  {
    ...
    WriteUint8(mem', address, w2 as u8)
  }
    ...
    decreases mem, address, val
  {
    ...
    WriteUint16(mem', address, w2 as u16)
  }
    ...
    decreases mem, address, val
  {
    ...
    WriteUint32(mem', address, w2 as u32)
  }
    ...
    decreases mem, address, val
  {
    ...
    WriteUint64(mem', address, w2 as u64)
  }
    ...
    decreases mem, address, val
  {
    ...
    WriteUint128(mem', address, w2 as u128)
  }
...
module EVM {
    ...
    decreases op, s
  {
    ...
    case SAR =>
      s.UseGas(G_VERYLOW)
    case KECCAK256 =>
      s.UseGas(CostExpandRange(s, 2, 0, 1))
    case ADDRESS =>
      s.UseGas(G_BASE)
    case BALANCE =>
    ...
    case CALLDATASIZE =>
      s.UseGas(G_BASE)
    case CALLDATACOPY =>
      s.UseGas(CostExpandRange(s, 3, 0, 2))
    case CODESIZE =>
      s.UseGas(G_BASE)
    case CODECOPY =>
      s.UseGas(CostExpandRange(s, 3, 0, 2))
    ...
    case EXTCODESIZE =>
      s.UseGas(CostExtAccount(s))
    case EXTCODECOPY =>
      s.UseGas(CostExpandRange(s, 4, 1, 3))
    case RETURNDATASIZE =>
      s.UseGas(G_BASE)
    case RETURNDATACOPY =>
      s.UseGas(CostExpandRange(s, 3, 0, 2))
    ...
    case POP =>
      s.UseGas(G_BASE)
    case MLOAD =>
      s.UseGas(CostExpandBytes(s, 1, 0, 32))
    case MSTORE =>
      s.UseGas(CostExpandBytes(s, 2, 0, 32))
    case MSTORE8 =>
      s.UseGas(CostExpandBytes(s, 2, 0, 1))
    case SLOAD =>
    ...
    case TSTORE =>
      s.UseGas(G_WARMACCESS)
    case MCOPY =>
      s.UseGas(CostExpandDoubleRange(s, 3, 0, 2, 1, 2))
    case PUSH0 =>
      s.UseGas(G_BASE)
    case PUSH1 =>
      s.UseGas(G_VERYLOW)
    ...
    case LOG0 =>
      s.UseGas(CostExpandRange(s, 2, 0, 1))
    case LOG1 =>
      s.UseGas(CostExpandRange(s, 3, 0, 1))
    case LOG2 =>
      s.UseGas(CostExpandRange(s, 4, 0, 1))
    case LOG3 =>
      s.UseGas(CostExpandRange(s, 5, 0, 1))
    case LOG4 =>
      s.UseGas(CostExpandRange(s, 6, 0, 1))
    case CREATE =>
      s.UseGas(CostExpandRange(s, 3, 1, 2))
    case CALL =>
      s.UseGas(CostExpandDoubleRange(s, 7, 3, 4, 5, 6))
    case CALLCODE =>
      s.UseGas(CostExpandDoubleRange(s, 7, 3, 4, 5, 6))
    case RETURN =>
      s.UseGas(CostExpandRange(s, 2, 0, 1))
    case DELEGATECALL =>
      s.UseGas(CostExpandDoubleRange(s, 6, 2, 3, 4, 5))
    case CREATE2 =>
      s.UseGas(CostExpandRange(s, 4, 1, 2))
    case STATICCALL =>
      s.UseGas(CostExpandDoubleRange(s, 6, 2, 3, 4, 5))
    case REVERT =>
      s.UseGas(CostExpandRange(s, 2, 0, 1))
...
module Gas {
  ...
  function ExpansionSize(mem: Memory.T, address: nat, len: nat): nat
    decreases mem, address, len
  {
    if len == 0 || address - 1 < |mem.contents| then
      0
    else
      var before: int := |mem.contents| / 32; var after: int := Memory.SmallestLarg32(address - 1) / 32; QuadraticCostIsMonotonic(after, before); assert QuadraticCost(after) >= QuadraticCost(before); QuadraticCost(after) - QuadraticCost(before)
  }
    ...
    decreases st
  {
    ...
    if st.Gas() <= G_CALLSTIPEND then
      ...
      CostSStoreCalc(originalVal, currentVal, newVal)
    else
      G_ZERO
  }
...
module SubState {
  ...
  datatype Raw = SubState(selfDestruct: set<u160>, log: seq<LogEntry>, touched: set<(u160, u256)>, refund: int, accessedAccounts: set<u160>, accessedKeys: set<(u160, u256)>) {
    ...
    function AccountAccessed(account: u160): Raw
      decreases this, account
    {
      var naccessed: set<u160> := accessedAccounts;
    ...
    function KeyAccessed(account: u160, address: u256): Raw
      decreases this, account, address
    {
      var naccessed: set<(u160, u256)> := accessedKeys;
...
module WorldState {
  ...
  datatype T = WorldState(accounts: map<u160, Account>, pretransactionaccounts: map<u160, Account>) {
    ...
    {
      var entry: Account := accounts[account];
      var nBalance: u256 := entry.balance;
      this.(accounts := this.accounts[account := entry.(balance := nBalance)])
    }
    ...
    {
      var entry: Account := accounts[account];
      this.(accounts := this.accounts[account := entry.(nonce := entry.nonce)])
    }
...
module EvmFork {
    ...
    decreases eips, bytecodes
  {
    if |eips| == 0 then
      ...
      match eips[0] case 1153 => EipBytecodes(eips[1..], bytecodes) case 3198 => EipBytecodes(eips[1..], bytecodes) case 3855 => EipBytecodes(eips[1..], bytecodes) case 5656 => EipBytecodes(eips[1..], bytecodes) case _ /* _v1 */ => EipBytecodes(eips[1..], bytecodes)
  }

  const BERLIN_EIPS: seq<nat> := [2565, 2929, 2718, 2930]
  const LONDON_EIPS: seq<nat> := BERLIN_EIPS
  const SHANGHAI_EIPS: seq<nat> := LONDON_EIPS
  const CANCUN_EIPS: seq<nat> := SHANGHAI_EIPS
...
module Memory {
    ...
    decreases mem, address
  {
    if address < |mem.contents| then
      ...
      var extLength: nat := SmallestLarg32(address); mem.(contents := mem.contents)
  }
...
module EvmState {
  ...
  datatype State = EXECUTING(evm: T) | ERROR(error: Error, gas: nat := 0, data: Array<u8> := []) | RETURNS(gas: nat, data: Array<u8>, world: WorldState.T, transient: TransientStorage.T, substate: SubState.T) | CONTINUING(Continuation) {
      ...
      decreases this, address, len
    {
      if len == 0 then
        ...
        var last: int := address - 1; EXECUTING(evm.(memory := Memory.Expand(evm.memory, last)))
    }
  ...
  datatype Continuation = CALLS(evm: T, sender: u160, recipient: u160, code: u160, gas: nat, callValue: u256, delegateValue: u256, callData: seq<u8>, writePermission: bool, outOffset: nat, outSize: nat) | CREATES(evm: T, gas: nat, endowment: u256, initcode: seq<u8>, salt: Option<u256>) {
      ...
      decreases this, depth
    {
      ...
      Call(evm.world, evm.transient, ctx, evm.fork, precompiled, evm.substate, code, callValue, gas, depth)
    }
      ...
      decreases this, depth, address, initcode
    {
      ...
      Create(evm.world, evm.transient, ctx, evm.fork, precompiled, evm.substate, initcode, gas, depth)
    }
...
module Bytecode {
    ...
    decreases st
  {
    if st.Operands() >= 3 then
      ...
      var res: int := if rem == 0 then 0 else lhs % rem;
    ...
    decreases st
  {
    if st.Operands() >= 3 then
      ...
      var d_loc: nat := st.Peek(1) as nat;
      var len: nat := st.Peek(2) as nat;
      if d_loc <= st.evm.context.ReturnDataSize() as nat then
  ...
  function RJump(st: ExecutingState, offset: i16): (st': State)
    decreases st, offset
  {
    var post_pc: int := st.evm.pc as int;
    var target: int := post_pc;
    ...
    decreases st, offset
  {
    if st.Operands() >= 1 then
      ...
      if val == 0 then
        ...
        var post_pc: int := st.evm.pc as int; var target: int := post_pc; if st.IsInstructionBoundary(target) then st.Pop(1).Goto(target as u256) else ERROR(INVALID_JUMPDEST)
    else
      ERROR(STACK_UNDERFLOW)
  }
  ...
  {
    if st.Capacity() >= 1 then
      var bytes: seq<u8> := Code.Slice(st.evm.code, st.evm.pc, k);
      assert 0 < |bytes| <= 32;
      var val: u256 := ByteUtils.ConvertBytesTo256(bytes);
      st.Push(val).Skip(|bytes|)
    else
      ERROR(STACK_OVERFLOW)
  }
  ...
  function LogN(st: ExecutingState, n: nat): (st': State)
    ...
    decreases st, n
  {
    if st.Operands() >= n then
      if !st.WritesPermitted() then
        ERROR(WRITE_PROTECTION_VIOLATED)
      else
        var m_loc: nat := st.Peek(0) as nat; var len: nat := st.Peek(1) as nat; var entry: (seq<u256>, seq<u8>) := (st.PeekN(n)[2..], Memory.Slice(st.evm.memory, m_loc, len)); st.Expand(m_loc, len).Log([entry]).Pop(n).Next()
    else
      ERROR(STACK_UNDERFLOW)
  }
