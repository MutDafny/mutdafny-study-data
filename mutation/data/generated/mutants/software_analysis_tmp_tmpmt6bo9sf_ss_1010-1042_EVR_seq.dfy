...
{
  |a| == |b| &&
  ((|a| == 0 && |b| == 0) || exists i: int, j: int {:trigger b[0 .. j], a[0 .. i]} {:trigger b[0 .. j], a[i]} {:trigger a[0 .. i], b[j]} {:trigger b[j], a[i]} :: 0 <= i < |a| && 0 <= j < |b| && a[i] == b[j] && is_permutation(a[0 .. i] + if i < |a| then a[i + 1..] else [], b[0 .. j] + []))
}
