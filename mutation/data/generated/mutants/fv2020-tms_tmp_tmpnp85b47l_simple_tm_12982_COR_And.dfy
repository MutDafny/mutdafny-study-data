    ...
    decreases input, pid
  {
    ...
    if state.currentOp == |tx.ops| {
      if state.currentSubOp == 0 {
        if !forall o: int {:trigger o in system.lockedObjs} {:trigger o in state.writeSet} {:trigger o in state.readSet} :: (o in state.readSet ==> o in state.writeSet) && (o in state.readSet ==> o !in system.lockedObjs) {
