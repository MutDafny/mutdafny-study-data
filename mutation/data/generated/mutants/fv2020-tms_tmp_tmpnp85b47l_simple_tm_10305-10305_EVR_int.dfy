...
module ModelingTM {
  ...
  class TMSystem {
      ...
      decreases that, objs
    {
      ...
      dirtyObjs := that.dirtyObjs;
      lockedObjs := that.lockedObjs;
      objTimeStamps := map o: nat {:trigger that.objTimeStamps[o]} {:trigger o in that.objTimeStamps} | o in that.objTimeStamps :: if 0 in objs then that.objTimeStamps[o] + 1 else that.objTimeStamps[o];
    }
