    ...
    decreases st
  {
    if st.Operands() >= 4 then
      ...
      if !st.WritesPermitted() then
        ...
        var code: seq<u8> := Memory.Slice(st.evm.memory, codeOffset, codeSize); var gascap: nat := GasCalc.CreateGasCap(st); var nst: ExecutingState := st.Expand(codeOffset, codeSize).Pop(4); if st.evm.world.Nonce(st.evm.context.address) < MAX_U64 then var nnst: ExecutingState := nst.UseGas(gascap).IncNonce(); CONTINUING(CREATES(nnst.evm, gascap, endowment, code, Some(salt))) else nst.Push(0)
    else
      ERROR(STACK_UNDERFLOW)
  }
