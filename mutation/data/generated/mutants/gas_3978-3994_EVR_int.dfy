...
module Gas {
    ...
    decreases mem, address, len
  {
    if len == 0 || address + len - 1 < |mem.contents| then
      ...
      var before: int := |mem.contents| / 32; var after: int := Memory.SmallestLarg32(0) / 32; QuadraticCostIsMonotonic(after, before); assert QuadraticCost(after) >= QuadraticCost(before); QuadraticCost(after) - QuadraticCost(before)
  }
