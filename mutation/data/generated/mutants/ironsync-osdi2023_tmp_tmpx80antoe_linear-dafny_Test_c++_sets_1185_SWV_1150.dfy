...
method SetSeq()
{
  ...
  var m3: seq<uint32> := [1, 2, 3, 4];
  var m4: seq<uint32> := [1, 2, 3];
  var n1: seq<uint32> := [1];
...
method SetComprehension(s: set<uint32>)
  requires forall i: uint32 {:trigger i in s} :: 0 <= i < 10 ==> i in s
  requires |s| == 10
  decreases s
{
  var t: set<uint32> := set y: uint32 {:trigger y in s} | y in s;
