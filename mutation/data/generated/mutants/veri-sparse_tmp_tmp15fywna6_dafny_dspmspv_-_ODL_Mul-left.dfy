  ...
  decreases X_val, X_crd, X_pos, X_crd1, X_len, v_val, v_crd
{
    ...
    decreases pX_end1 - n
  {
      ...
      decreases pX_end + pV_end - (kX + kV)
    {
      ...
      k := min(kV0, kX0);
      if kX0 == k && kV0 == k {
        y[X_crd1[n]] := y[X_crd1[n]] + v_val[kV];
        kX := kX + 1;
        kV := kV + 1;
      } else if kX0 == k {
        kX := kX + 1;
...
method Main()
{
  ...
  var X_crd := new nat[4] ((i: nat) => if i <= 3 then 2 else 0);
  var X_pos := new nat[5] ((i: nat) => i);
  var X_crd1 := new nat[4] ((i: nat) => 2);
  var X_pos1 := new nat[2] ((i: nat) => 8);
  var X_len := 8;
  var v_val := new int[4] ((i: nat) => 30 + i);
  var v_crd := new nat[4] ((i: nat) => 2);
