...
module Bytecode {
    ...
    decreases st
  {
    if st.Operands() >= 3 then
      ...
      if !st.WritesPermitted() then
        ERROR(WRITE_PROTECTION_VIOLATED)
      else if st.IsEipActive(3860) then
...
module Arrays {
    ...
    decreases src, dst, start
  {
    ...
    seq(|dst|, (i: int) requires i >= 0 => if i >= start then src[i - start] else dst[i])
  }
...
module EvmState {
    ...
    decreases world, transient, ctx, fork, precompiled, substate, initcode, gas, depth
  {
    ...
    if depth > 1024 || !world.CanWithdraw(ctx.sender, endowment) || !ctx.writePermission then
      ERROR(REVERTS, gas)
    else if world.Exists(ctx.address) then
...
module Precompiled {
    ...
    decreases lB, lE, data
  {
    if lE <= 32 then
      ...
      var w: u256 := ByteUtils.ReadUint256(data, 96 + lB); var g: int := 8 * (lE - 32); if 32 < lE then g + U256.Log2(w) else g
  }
