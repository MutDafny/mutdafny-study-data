...
module Bytecode {
    ...
    decreases st
  {
    if st.Operands() >= 1 then
      ...
      var ss: Raw := st.evm.substate.AccountAccessed(r).AccountDestructed(address);
      var w: T := if address != r && (!st.Exists(r) || st.evm.world.CanDeposit(r, balance)) then st.evm.world.EnsureAccount(r).Transfer(address, r, balance) else st.evm.world.Withdraw(address, balance);
