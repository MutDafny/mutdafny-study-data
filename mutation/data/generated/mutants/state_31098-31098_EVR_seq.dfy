      ...
      decreases this, vm
    {
      ...
      if st.Capacity() >= 1 then
        ...
        if vm.ERROR? && vm.error != REVERTS then
          ...
          var m: int := Min(|vm.data|, outSize); var data: seq<u8> := vm.data[0 .. m]; var nst: State := if vm.RETURNS? then st.Merge(vm.world, vm.transient, vm.substate) else st; nst.Push(exitCode).Refund(vm.gas).SetReturnData(vm.data).Copy(outOffset, [])
      else
