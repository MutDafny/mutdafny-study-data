...
module Gas {
  ...
  function CallCost(st: ExecutingState): nat
    decreases st
  {
    -if st.Operands() >= 7 then var value: nat := st.Peek(2) as nat; var to: u160 := (st.Peek(1) as int % TWO_160) as u160; CostAccess(st, to) + CostCallXfer(value) + CostCallNew(st, to, value) else G_ZERO
  }
