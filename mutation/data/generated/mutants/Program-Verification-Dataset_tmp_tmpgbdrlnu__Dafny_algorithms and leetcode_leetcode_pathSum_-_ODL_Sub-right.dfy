...
method hasPathSum(root: TreeNode, targetSum: int) returns (b: bool)
  ensures b ==> exists p: seq<TreeNode> {:trigger pathSum(p)} {:trigger isPath(p, root)} :: isPath(p, root) && pathSum(p) == targetSum
  decreases root, targetSum
{
  if root == Nil {
    return false;
  }
  ...
  var leftPath := hasPathSum(root.left, targetSum);
  var rightPath := hasPathSum(root.right, targetSum);
  if leftPath {
