...
lemma /*{:_inductionTrigger a + b}*/ /*{:_induction a, b}*/ sumUpLemma(a: seq<int>, b: seq<int>)
...
lemma /*{:_inductionTrigger sumUp(nums)}*/ /*{:_inductionTrigger sum(nums)}*/ /*{:_induction nums}*/ sumsEqual(nums: seq<int>)
  ...
  decreases |nums|
{
  if nums == [] {
    ...
    calc {
      ...
      {
        assert forall a: int {:trigger [a]} :: sumUp([a]) == a;
      }
...
method FindPivotIndex(nums: seq<int>) returns (index: int)
  requires |nums| > 0
  ensures index == -1 ==> forall k: nat {:trigger nums[0 .. k]} :: k < |nums| ==> sum(nums[0 .. k]) != sum(nums[k + 1..])
  ...
  decreases nums
{
  ...
  while i < |nums| + 1
    ...
    invariant forall k: nat {:trigger nums[0 .. k]} {:trigger leftsums[k]} :: 0 <= k < i && k <= |nums| ==> leftsums[k] == sum(nums[0 .. k])
    invariant forall k: nat {:trigger rightsums[k]} :: 0 <= k < i && k <= |nums| ==> rightsums[k] == sumUp(nums[|nums| - k..])
    decreases |nums| + 1 - i
  {
    leftsums := leftsums + [leftsums[i - 1] + nums[i - 1]];
    assert nums[0 .. i] == nums[0 .. i - 1] + [nums[i - 1]];
    rightsums := rightsums + [nums[|rightsums| - i] + rightsums[i - 1]];
    i := i + 1;
  }
  assert forall k: nat {:trigger rightsums[k]} :: 0 <= k < i && k <= |nums| ==> rightsums[k] == sum(nums[|nums| - k..]) by {
  ...
  while i < |nums|
    invariant 0 <= i <= |nums|
    invariant forall k: nat {:trigger nums[0 .. k]} :: k < i ==> sum(nums[0 .. k]) != sum(nums[k + 1..])
    decreases |nums| - i
  {
    var x := |nums| - (i + 1);
