...
module U256 {
    ...
    decreases v, k
  {
    if k >= 31 then
      ...
      var ith: int := 31 - k; var byte: u8 := NthUint8(v, ith); var signs: seq<u8> := if byte >= 128 then seq(31 - k, (i: int) => 255) else seq(31 - k, (i: int) => 0); var bytes: seq<u8> := ToBytes(v)[ith..]; assert |signs + bytes| == 32; Read(signs + bytes, -1)
  }
