...
{
  if j: int, i: int {:trigger a[j], a[i]} :| 0 <= i < j == a.Length && a[i] > a[j] {
    ghost var bef := inversions(a);
    a[i], a[j] := a[j], a[i];
    ghost var aft := inversions(a);
    ghost var aft2bef := map p: (int, nat) {:trigger p.1} {:trigger p.0} {:trigger p in aft} | p in aft :: (if p.0 == i && p.1 > j then j else if p.0 == j then i else p.0, if p.1 == i then j else if p.1 == j && p.0 < i then i else p.1);
