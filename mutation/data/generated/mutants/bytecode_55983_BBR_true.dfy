    ...
    decreases st
  {
    if st.Operands() >= 3 then
      ...
      if !st.WritesPermitted() then
        ...
        var code: seq<u8> := Memory.Slice(st.evm.memory, codeOffset, codeSize); var gascap: nat := GasCalc.CreateGasCap(st); var nst: ExecutingState := st.Expand(codeOffset, codeSize).Pop(3).Next(); if true then var nnst: ExecutingState := nst.UseGas(gascap).IncNonce(); CONTINUING(CREATES(nnst.evm, gascap, endowment, code, None)) else nst.Push(0)
    else
      ERROR(STACK_UNDERFLOW)
  }
