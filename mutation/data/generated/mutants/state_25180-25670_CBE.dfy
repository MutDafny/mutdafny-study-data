...
module EvmState {
    ...
    decreases world, transient, ctx, fork, precompiled, substate, codeAddress, value, gas, depth
  {
    ...
    if depth > 1024 || !world.CanWithdraw(ctx.sender, value) then
      ...
      var w: T := world.EnsureAccount(address); if !w.CanDeposit(address, value) then ERROR(BALANCE_OVERFLOW) else var nw: T := w.Transfer(ctx.sender, address, value); var cod: Code.T := w.GetOrDefault(codeAddress).code; if |cod.contents| == 0 then RETURNS(gas, [], nw, transient, substate) else var stack: EvmStack := EmptyEvmStack; var mem: T := Memory.Create(); var evm: Raw := EVM(fork, ctx, precompiled, nw, stack, mem, transient, cod, substate, gas, 0); EXECUTING(evm)
  }
