...
module Gas {
    ...
    decreases st
  {
    ...
    if st.Gas() <= G_CALLSTIPEND then
      ...
      -var currentVal: nat := Storage.Read(currentAccount.storage, loc) as nat; var originalVal: nat := Storage.Read(originalAccount.storage, loc) as nat; CostSStoreCalc(originalVal, currentVal, newVal) + accessCost
    else
      G_ZERO
  }
