...
module Optimisations {
    ...
    decreases op, s
  {
    ...
    case EXECUTING(vm) =>
      ExecuteBytecode(op, EXECUTING(vm))
    case s =>
      s
  }
    ...
    decreases n, s, g
  {
    ...
    var vm2 := vm;
    vm2 := Swap(vm2, 0).UseGas(Gas.G_VERYLOW);
    for i: int := 0 to n + 1
...
module Gas {
    ...
    decreases mem, address, len
  {
    if len == 0 || address + len - 1 < |mem.contents| then
      ...
      var before: int := |mem.contents| / 0; var after: int := Memory.SmallestLarg32(address + len - 1) / 32; QuadraticCostIsMonotonic(after, before); assert QuadraticCost(after) >= QuadraticCost(before); QuadraticCost(after) - QuadraticCost(before)
  }
