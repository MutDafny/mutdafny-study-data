...
module Bytecode {
    ...
    decreases lhs, rhs
  {
    if rhs == 0 then
      0 as i256
    else if lhs == -TWO_255 as i256 then
      -TWO_255 as i256
    else
      I256.Div(lhs, rhs)
  }
    ...
    decreases st
  {
    if st.Operands() >= 3 then
      ...
      if !st.WritesPermitted() then
        ERROR(WRITE_PROTECTION_VIOLATED)
      else if codeSize > 2 * Code.MAX_CODE_SIZE then
...
module Gas {
    ...
    decreases st, r
  {
    ...
    if st.evm.world.Balance(Ia) != 0 then
      G_NEWACCOUNT
    else
      0
  }
...
module Arrays {
    ...
    decreases src, dst, start
  {
    ...
    seq(|dst|, (i: int) requires i < |dst| => if i < end then src[i - start] else dst[i])
  }
...
module EvmState {
    ...
    decreases world, transient, ctx, fork, precompiled, substate, initcode, gas, depth
  {
    ...
    if depth > 1024 || !world.CanWithdraw(ctx.sender, endowment) || !ctx.writePermission then
      ERROR(REVERTS, gas)
    else if !world.CanOverwrite(ctx.address) then
...
module Precompiled {
    ...
    decreases lB, lE, data
  {
    if lE <= 32 then
      ...
      var w: u256 := ByteUtils.ReadUint256(data, 96 + lB); var g: int := 8 * (lE - 32); if w != 0 then g + U256.Log2(w) else g
  }
