...
module Transient {
  ...
  const LOCK_CONTRACT: seq<u8> := [PUSH0, TLOAD, PUSH2, 0, 30, JUMPI, PUSH1, 1, PUSH0, TSTORE, PUSH0, SLOAD, PUSH1, 1, ADD, PUSH0, SSTORE, PUSH0, PUSH0, PUSH0, PUSH0, PUSH0, PUSH0, CALLDATALOAD, GAS, CALL, PUSH0, PUSH0, TSTORE, STOP, JUMPDEST, PUSH0, PUSH0, REVERT]

  method block_0_0x0000(st': EvmState.ExecutingState) returns (st'': EvmState.State)
    requires st'.evm.code == Code.Create(LOCK_CONTRACT)
    requires st'.WritesPermitted() && st'.PC() == 0 && st'.Gas() <= MAX_U256 as nat
    ...
    decreases st'
  {
    ...
    st := TLoad(st);
    st := Push2(st, 30);
    assume {:axiom} st.IsJumpDest(30);
    st := JumpI(st);
    if st.PC() == 30 {
      st := block_0_0x001e(st);
      return st;
    }
    st := Push1(st, 1);
    st := Push0(st);
  ...
  method block_0_0x000b(st': EvmState.ExecutingState) returns (st'': EvmState.State)
    requires st'.evm.code == Code.Create(LOCK_CONTRACT)
    requires st'.WritesPermitted() && st'.PC() == 11 && st'.Gas() <= MAX_U256 as nat
  ...
  method block_0_0x0014(st': EvmState.ExecutingState) returns (st'': EvmState.State)
    requires st'.evm.code == Code.Create(LOCK_CONTRACT)
    requires st'.WritesPermitted() && st'.PC() == 20 && st'.Gas() <= MAX_U256 as nat
    requires st'.Operands() == 3
    requires st'.Peek(0) == 0 && st'.Peek(1) == 0 && st'.Peek(2) == 0
    requires LockAcquired(st', st'.evm.context.address)
    ensures st''.EXECUTING? ==> st''.evm.context == st'.evm.context
    ensures st''.RETURNS? ==> st'.evm.context.address in st''.world.accounts.Keys
    ...
    decreases st'
  {
    ...
    var CONTINUING(cc) := Bytecode.Call(st);
    {
      ...
      if inner.EXECUTING? {
        inner := external_call(cc.sender, inner);
      }
  ...
  method block_0_0x001c(st': EvmState.ExecutingState) returns (st'': EvmState.State)
    requires st'.evm.code == Code.Create(LOCK_CONTRACT)
    requires st'.WritesPermitted() && st'.PC() == 28
    requires st'.Operands() == 3
    requires st'.Peek(0) == 0 && st'.Peek(1) == 0
    ensures st''.RETURNS?
  ...
  method block_0_0x001e(st': EvmState.ExecutingState) returns (st'': EvmState.State)
    requires st'.evm.code == Code.Create(LOCK_CONTRACT)
    requires st'.WritesPermitted() && st'.PC() == 30
    requires st'.Operands() == 0
    ensures st''.ERROR?
    decreases st'
  {
  ...
  method external_call(sender: u160, st: EvmState.ExecutingState) returns (st': EvmState.TerminatedState)
    ...
    decreases sender, st
  {
    return EvmState.ERROR(EvmState.INSUFFICIENT_GAS);
  }

  function LockAcquired(st: EvmState.ExecutingState, account: u160): bool
    decreases st, account
  {
