...
method NinetyOne(x: int, ghost proveFunctionalPostcondition: bool) returns (z: int)
  ...
  decreases x, proveFunctionalPostcondition
{
  var y1 := x;
  var y2 := 1;
  while true
  ...
  decreases X, M
{
  ...
  while a != M
    invariant 1 <= a <= M
    decreases if a <= M then M - a else a - M
  {
    var b := a + 1;
    while b != M + 1
      invariant a + 1 <= b <= M + 1
      decreases if b <= M + 1 then M + 1 - b else b - (M + 1)
    {
      var c := M;
      while c != a
        invariant a <= c <= M
        decreases if c <= a then a - c else c - a
      {
        assume X[a - 1, a - 1] != 0;
        X[b - 1, c - 1] := X[y - 1, c - 1] - X[b - 1, a - 1] / X[a - 1, a - 1] * X[a - 1, c - 1];
        c := c - 1;
      }
      b := b + 1;
    }
