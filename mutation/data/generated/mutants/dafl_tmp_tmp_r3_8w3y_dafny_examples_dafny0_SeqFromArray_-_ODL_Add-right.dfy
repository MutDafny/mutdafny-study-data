...
{
  if {
    case l <= c.Length && forall i: int {:trigger a[i]} :: 0 <= i < m ==> a[i] == c[l] =>
      assert a[..m] == c[l .. l + m];
    case l <= c.Length && forall i: int {:trigger a[i]} :: k <= i < a.Length ==> a[i] == c[l] =>
      assert a[k..] == c[l + k .. l + a.Length];
    case l <= c.Length && forall i: int {:trigger a[i]} :: k <= i < k + m ==> a[i] == c[l] =>
