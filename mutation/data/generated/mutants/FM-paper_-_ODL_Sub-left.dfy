...
module Kontract1 {
  const INC_CONTRACT := Code.Create([PUSH1, 0, SLOAD, PUSH1, 1, ADD, DUP1, PUSH1, 15, JUMPI, PUSH1, 0, PUSH1, 0, REVERT, JUMPDEST, PUSH1, 0, SSTORE, STOP])
  ...
  const OVERFLOW_CHECK := Code.Create([DUP2, ADD, LT, PUSH1, 7, JUMPI, STOP, JUMPDEST, PUSH1, 0, PUSH1, 0, REVERT])

  method {:verify false} OverflowCheck(st: ExecutingState, x: u256, y: u256)
      returns (st': State)
    requires st.PC() == 0 && st.Fork() == EvmFork.BERLIN
  ...
  method {:verify false} Loopy(st: ExecutingState, c: u8) returns (st': State)
    ...
    requires st.Gas() >= 3 * Gas.G_VERYLOW + Gas.G_JUMPDEST + c as nat * (2 * Gas.G_HIGH + 2 * Gas.G_JUMPDEST + 6 * Gas.G_VERYLOW) + Gas.G_HIGH
    requires st.evm.code == Code.Create([PUSH1, c, JUMPDEST, DUP1, PUSH1, 8, JUMPI, STOP, JUMPDEST, PUSH1, 1, SWAP1, SUB, PUSH1, 2, JUMP])
    ensures st'.RETURNS?
    decreases st, c
  {
...
module EVM {
    ...
    decreases op, s
  {
    ...
    case SAR =>
      s.UseGas(G_VERYLOW)
    case KECCAK256 =>
      s.UseGas(CostExpandRange(s, 2, 0, 1) + CostKeccak256(s))
    case ADDRESS =>
      s.UseGas(G_BASE)
    case BALANCE =>
    ...
    case CALLDATASIZE =>
      s.UseGas(G_BASE)
    case CALLDATACOPY =>
      s.UseGas(CostExpandRange(s, 3, 0, 2) + G_VERYLOW + CostCopy(s, 2))
    case CODESIZE =>
      s.UseGas(G_BASE)
    case CODECOPY =>
      s.UseGas(CostExpandRange(s, 3, 0, 2) + G_VERYLOW + CostCopy(s, 2))
    ...
    case EXTCODESIZE =>
      s.UseGas(CostExtAccount(s))
    case EXTCODECOPY =>
      s.UseGas(CostExpandRange(s, 4, 1, 3) + CostExtAccount(s) + CostCopy(s, 3))
    case RETURNDATASIZE =>
      s.UseGas(G_BASE)
    case RETURNDATACOPY =>
      s.UseGas(CostExpandRange(s, 3, 0, 2) + G_VERYLOW + CostCopy(s, 2))
    ...
    case POP =>
      s.UseGas(G_BASE)
    case MLOAD =>
      s.UseGas(CostExpandBytes(s, 1, 0, 32) + G_VERYLOW)
    case MSTORE =>
      s.UseGas(CostExpandBytes(s, 2, 0, 32) + G_VERYLOW)
    case MSTORE8 =>
      s.UseGas(CostExpandBytes(s, 2, 0, 1) + G_VERYLOW)
    case SLOAD =>
    ...
    case TSTORE =>
      s.UseGas(G_WARMACCESS)
    case MCOPY =>
      s.UseGas(CostExpandDoubleRange(s, 3, 0, 2, 1, 2) + G_VERYLOW + CostCopy(s, 2))
    case PUSH0 =>
      s.UseGas(G_BASE)
    case PUSH1 =>
      s.UseGas(G_VERYLOW)
    ...
    case SWAP16 =>
      s.UseGas(G_VERYLOW)
    case LOG0 =>
      s.UseGas(CostExpandRange(s, 2, 0, 1) + CostLog(s, 0))
    case LOG1 =>
      s.UseGas(CostExpandRange(s, 3, 0, 1) + CostLog(s, 1))
    case LOG2 =>
      s.UseGas(CostExpandRange(s, 4, 0, 1) + CostLog(s, 2))
    case LOG3 =>
      s.UseGas(CostExpandRange(s, 5, 0, 1) + CostLog(s, 3))
    case LOG4 =>
      s.UseGas(CostExpandRange(s, 6, 0, 1) + CostLog(s, 4))
    case CREATE =>
      s.UseGas(CostExpandRange(s, 3, 1, 2) + CostCreate(s))
    case CALL =>
      s.UseGas(CostExpandDoubleRange(s, 7, 3, 4, 5, 6) + CallCost(s))
    case CALLCODE =>
      s.UseGas(CostExpandDoubleRange(s, 7, 3, 4, 5, 6) + CallCodeCost(s))
    case RETURN =>
      s.UseGas(CostExpandRange(s, 2, 0, 1) + G_ZERO)
    case DELEGATECALL =>
      s.UseGas(CostExpandDoubleRange(s, 6, 2, 3, 4, 5) + DelegateCallCost(s))
    case CREATE2 =>
      s.UseGas(CostExpandRange(s, 4, 1, 2) + CostCreate2(s))
    case STATICCALL =>
      s.UseGas(CostExpandDoubleRange(s, 6, 2, 3, 4, 5) + StaticCallCost(s))
    case REVERT =>
      s.UseGas(CostExpandRange(s, 2, 0, 1) + G_ZERO)
    case SELFDESTRUCT =>
...
module Gas {
  ...
  {
    if len == 0 || 1 < |mem.contents| then
      0
    else
      var before: int := |mem.contents| / 32; var after: int := Memory.SmallestLarg32(1) / 32; QuadraticCostIsMonotonic(after, before); assert QuadraticCost(after) >= QuadraticCost(before); QuadraticCost(before)
  }
...
module Arrays {
    ...
    decreases mem, address, len
  {
    ...
    if n <= |mem| then
      ...
      mem[address..] + seq(|mem|, (i: int) => padding)
    else
    ...
    decreases src, dst, start
  {
    ...
    seq(|dst|, (i: int) requires i >= 0 && i < |dst| => if i >= start && i < end then src[start] else dst[i])
  }
...
module Int {
  ...
  const MAX_I128: int := 1
  const MIN_I256: int := -TWO_255
  const MAX_I256: int := 1
  const MAX_U1: int := 1
  const MAX_U2: int := 1
  const MAX_U3: int := 1
  const MAX_U4: int := 1
  ...
  const MAX_U88: int := 1
  const MAX_U96: int := 1
  const MAX_U104: int := 1
  const MAX_U112: int := 1
  const MAX_U120: int := 1
  const MAX_U128: int := 1
  const MAX_U136: int := 1
  const MAX_U144: int := 1
  const MAX_U152: int := 1
  const MAX_U160: int := 1
  const MAX_U168: int := 1
  const MAX_U176: int := 1
  const MAX_U184: int := 1
  const MAX_U192: int := 1
  const MAX_U200: int := 1
  const MAX_U208: int := 1
  const MAX_U216: int := 1
  const MAX_U224: int := 1
  const MAX_U232: int := 1
  const MAX_U240: int := 1
  const MAX_U248: int := 1
  const MAX_U256: int := 1
...
module U256 {
    ...
    decreases v, k
  {
    if k >= 31 then
      ...
      var ith: nat := k; var byte: u8 := NthUint8(v, ith); var signs: seq<u8> := if byte >= 128 then seq(k, (i: int) => 255) else seq(k, (i: int) => 0); var bytes: seq<u8> := ToBytes(v)[ith..]; assert |signs + bytes| == 32; Read(signs + bytes, 0)
  }
...
module Memory {
    ...
    decreases mem, address
  {
    if address < |mem.contents| then
      ...
      var extLength: nat := SmallestLarg32(address); mem.(contents := mem.contents + ByteUtils.Padding(|mem.contents|))
  }
  ...
  datatype State = EXECUTING(evm: T) | ERROR(error: Error, gas: nat := 0, data: Array<u8> := []) | RETURNS(gas: nat, data: Array<u8>, world: WorldState.T, transient: TransientStorage.T, substate: SubState.T) | CONTINUING(Continuation) {
      ...
      decreases this, address, len
    {
      if len == 0 then
        ...
        var last: int := 1; EXECUTING(evm.(memory := Memory.Expand(evm.memory, last)))
    }
...
module Precompiled {
    ...
    decreases lB, lE, data
  {
    if lE <= 32 then
      ...
      var w: u256 := ByteUtils.ReadUint256(data, 96 + lB); var g: int := 8 * 32; if 32 < lE && w != 0 then g + U256.Log2(w) else g
  }
