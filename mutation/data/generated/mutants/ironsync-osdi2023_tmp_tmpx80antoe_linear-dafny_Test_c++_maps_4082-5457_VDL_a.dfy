...
method TestNullsAmongValues()
{
  ...
  var d := new MyClass("jack");
  var e := new MyClass("sibylla");
  var m: map<uint32, MyClass?> := map[1 := b, 2 := null, 3 := null];
  var n: map<uint32, MyClass?> := map[0 := d, 10 := b, 20 := null, 4 := e];
  var o: map<uint32, MyClass?> := map[199 := null];
  var o': map<uint32, MyClass?> := map[199 := b, 55 := null];
  var o'': map<uint32, MyClass?> := map[199 := b, 56 := null];
