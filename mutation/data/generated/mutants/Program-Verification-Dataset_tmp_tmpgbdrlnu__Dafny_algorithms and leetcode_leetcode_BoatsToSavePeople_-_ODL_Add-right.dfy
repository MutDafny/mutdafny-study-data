  ...
  decreases people, limit
{
  ...
  {
    if people[upper] == limit || people[upper] > limit {
      boats := boats;
      assert isSafeBoat([people[upper]], limit);
      safeBoats := [[people[upper]]];
      ...
      boats := boats;
      if lower == upper {
        visitedLower := visitedLower;
        assert isSafeBoat([people[lower]], limit);
        safeBoats := [[people[lower]]];
      } else {
        ghost var gu := people[upper + 1..];
        assert multiset(gu) == visitedUpper;
        visitedUpper := visitedUpper;
        ...
        safeBoats := [[people[upper], people[lower]]];
        upper := upper - 1;
        assert people[upper + 1..] == [people[upper + 1]] + gu;
      }
      lower := lower;
