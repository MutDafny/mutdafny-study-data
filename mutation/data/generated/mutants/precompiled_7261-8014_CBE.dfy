...
module Precompiled {
    ...
    decreases data
  {
    ...
    var output: Array<u8> := if lB == 0 && lM == 0 then [] else var M_bytes: seq<u8> := Arrays.SliceAndPad(data, 96 + lB + lE, lM, 0); var M: nat := Int.FromBytes(M_bytes); var B_bytes: seq<u8> := Arrays.SliceAndPad(data, 96, lB, 0); var E_bytes: seq<u8> := Arrays.SliceAndPad(data, 96 + lB, lE, 0); var E: nat := Int.FromBytes(E_bytes); var B: nat := Int.FromBytes(B_bytes); var modexp: nat := MathUtils.ModPow(B, E, M); var modexp_bytes: seq<u8> := Int.ToBytes(modexp); (Int.LemmaLengthToBytes(modexp, M); Int.LemmaLengthFromBytes(M, M_bytes); ByteUtils.LeftPad(modexp_bytes, lM));
