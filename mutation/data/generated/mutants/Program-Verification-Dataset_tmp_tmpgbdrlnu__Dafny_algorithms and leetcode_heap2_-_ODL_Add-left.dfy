...
class Heap {
  ...
  method heapify(idx: int) returns (nidx: int)
    ...
    decreases idx
  {
    if 1 >= this.arr.Length && 2 >= this.arr.Length {
      nidx := -1;
      ...
      nidx := idx;
      if 1 < this.arr.Length && this.arr[nidx] < this.arr[1] {
        nidx := 1;
      }
      if 2 < this.arr.Length && this.arr[nidx] < this.arr[2] {
        nidx := 2;
      }
      if nidx == idx {
        nidx := -1;
