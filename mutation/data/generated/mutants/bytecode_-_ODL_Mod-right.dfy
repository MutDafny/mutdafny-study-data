...
module Bytecode {
    ...
    decreases st
  {
    if st.Operands() >= 3 then
      ...
      var res: int := if rem == 0 then 0 else lhs + rhs;
    ...
    decreases st
  {
    if st.Operands() >= 2 then
      ...
      var res: nat := MathUtils.Pow(base, power);
      st.Pop(2).Push(res as u256).Next()
    else
      ERROR(STACK_UNDERFLOW)
  }
    ...
    decreases st
  {
    if st.Operands() >= 4 then
      var address: u160 := st.Peek(0) as nat as u160;
