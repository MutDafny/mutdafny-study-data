...
class BoundedQueue<T(0)> {
    ...
    ensures Valid() && fresh(Repr - old(Repr))
  {
    ...
    data[wr] := x;
    assert (wr == data.Length - 1 ==> contents == if rd <= 0 then data[rd .. 0] else data[rd..] + data[..0]) && (wr != data.Length - 1 ==> contents == if rd <= wr + 1 then data[rd .. wr + 1] else data[rd..] + data[..wr + 1]);
    if wr == data.Length - 1 {
