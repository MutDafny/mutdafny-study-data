  ...
  decreases a
{
  ...
  while c != a.Length
    ...
    invariant forall k: int, l: int {:trigger a[l], a[k]} :: 0 <= k < c <= l < a.Length ==> a[k] <= a[l]
    invariant sortedA(a, c)
    decreases if c <= a.Length then a.Length - c else c - a.Length
  {
    var m := lookForMin(a, c);
    a[m], a[c] := a[c], a[m];
    assert forall k: int {:trigger a[k]} :: c <= k < a.Length ==> a[k] >= a[c];
    c := c + 1;
  }
}
