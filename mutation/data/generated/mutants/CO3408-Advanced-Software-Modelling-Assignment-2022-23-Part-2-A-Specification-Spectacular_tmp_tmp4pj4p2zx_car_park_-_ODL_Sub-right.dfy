...
class {:autocontracts} CarPark {
    ...
    decreases car
  {
    ...
    if car in carPark {
      ...
      reservedCarPark := reservedCarPark;
      success := true;
    }
  }
  ...
  {
    if weekend {
      availableSpaces := normalSpaces + reservedSpaces;
    } else {
      availableSpaces := normalSpaces;
    }
  }
  ...
  method enterCarPark(car: string) returns (success: bool)
    ...
    decreases car
  {
    if |carPark| >= normalSpaces || car in carPark || car in reservedCarPark {
      return false;
    } else {
      carPark := carPark + {car};
      return true;
