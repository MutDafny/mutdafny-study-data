  ...
  decreases a, c, n, j
{
  ...
  if {
    case A[j] == C[j] =>
      assert a[j] == c[j];
    case forall i: int {:trigger C[i]} {:trigger A[i]} :: A[i] == C[i] =>
      assert a[j] == c[j];
    case forall i: int {:trigger C[i]} {:trigger A[i]} :: A[i] == C[i] =>
...
{
  if {
    case l + m <= c.Length && forall i: int {:trigger a[i]} :: a[i] == c[l + i] =>
      assert a[..m] == c[l .. l + m];
    case l + a.Length <= c.Length && forall i: int {:trigger a[i]} :: a[i] == c[l + i] =>
      assert a[k..] == c[l + k .. l + a.Length];
    case l + k + m <= c.Length && forall i: int {:trigger a[i]} :: a[i] == c[l + i] =>
