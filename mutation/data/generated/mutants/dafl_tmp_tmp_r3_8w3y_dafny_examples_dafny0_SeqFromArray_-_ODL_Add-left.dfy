...
{
  if {
    case m <= c.Length && forall i: int {:trigger c[i]} {:trigger a[i]} :: 0 <= i < m ==> a[i] == c[i] =>
      assert a[..m] == c[l .. l + m];
    case a.Length <= c.Length && forall i: int {:trigger c[i]} {:trigger a[i]} :: k <= i < a.Length ==> a[i] == c[i] =>
      assert a[k..] == c[l + k .. l + a.Length];
    case m <= c.Length && forall i: int {:trigger c[i]} {:trigger a[i]} :: k <= i < k + m ==> a[i] == c[i] =>
