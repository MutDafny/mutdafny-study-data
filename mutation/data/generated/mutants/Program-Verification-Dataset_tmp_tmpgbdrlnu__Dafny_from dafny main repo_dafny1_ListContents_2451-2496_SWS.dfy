// Program-Verification-Dataset_tmp_tmpgbdrlnu__Dafny_from dafny main repo_dafny1_ListContents.dfy
...
class Node<T> {
  ...
  method ReverseInPlace() returns (reverse: Node<T>)
    ...
    ensures forall i: int {:trigger reverse.List[i]} :: 0 <= i < |reverse.List| ==> reverse.List[i] == old(List)[|old(List)| - 1 - i]
  {
    var current := next;
    reverse := this;
    reverse.next := null;
    ...
    while current != null
      ...
      invariant current != null ==> current.Valid() && current in old(Repr) && current.Repr <= old(Repr) && current.Repr !! reverse.Repr
      invariant current != null ==> |old(List)| == |reverse.List| + |current.List| && current.List == old(List)[|reverse.List|..]
      invariant forall i: int {:trigger reverse.List[i]} :: 0 <= i < |reverse.List| ==> reverse.List[i] == old(List)[|reverse.List| - 1 - i]
