...
module SimulationChecks {
    ...
    decreases g
  {
    ...
    var vm1 := EVM.Init(gas := g, code := [PUSH1, 1, PUSH1, 2, ADD, POP, STOP]);
    var vm2 := EVM.Init(g);
  ...
  method main1(g: nat)
    requires g >= 3 * Gas.G_VERYLOW + Gas.G_BASE + Gas.G_ZERO
    decreases g
  {
    assume {:axiom} {PUSH1, ADD, POP, STOP} <= EvmFork.BERLIN_BYTECODES;
    var a: u8 := 1;
    var b: u8 := 2;
    var vm1 := EVM.Init(gas := g, code := [PUSH1, a, PUSH1, a, ADD, POP, STOP]);
    var vm2 := EVM.Init(g);
    assert equiv(vm1, vm2);
    ...
    decreases c, g
  {
    ...
    var a: u8 := 1;
    var b: u8 := 2;
    var vm := EVM.Init(gas := g, code := [PUSH1, c, JUMPDEST, DUP1, ISZERO, DUP1, PUSH1, end, JUMPI, POP, PUSH1, 1, SWAP1, SUB, PUSH1, 2, JUMP, JUMPDEST, STOP]);
    ...
    while vm.Peek(0) == 0
      ...
      invariant vm.evm.code.contents == code_.contents
      invariant |vm.evm.stack.contents| > 1
      invariant vm.Capacity() >= 2
      ...
      decreases count
    {
      ...
      assert vm.CodeAtPC() == PUSH1;
      assert vm.CodeAtIndex(vm.PC() + 1) == 1;
      ...
      assert vm.CodeAtPC() == PUSH1;
      assert vm.CodeAtIndex(vm.PC() + 1) == 2;
