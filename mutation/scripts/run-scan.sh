...
#   --mutation_operator <AOR|ROR|COR|LOR|SOR|BBR|AOI|COI|LOI|AOD|COD|LOD|LVR|EVR|VER|LSR|LBI|MRR|MAP|MNR|MCR|MVR|SAR|CIR|CBR|CBE|TAR|DCR|SDL|VDL|SLD|ODL|THI|THD|AMR|MMR|FAR|PRV|SWS|SWV>
#   --output_dir_path <full path, e.g., $SCRIPT_DIR/../data/generated/scan/data/<mutation operator, e.g., AOR>/<Benchmark name, e.g., DafnyBench>/<Dafny program name, e.g., 630-dafny_tmp_tmpz2kokaiq_Solution>/>
#   [help]
# ------------------------------------------------------------------------------
...
if [ "$MUTATION_OPERATOR" == "ALL" ]; then
  MUTATION_OPERATOR_ARG="AOR ROR COR LOR SOR BBR AOI COI LOI AOD COD LOD LVR EVR VER LSR LBI MRR MAP MNR MCR MVR SAR CIR CBR CBE TAR DCR SDL VDL SLD ODL THI THD AMR MMR FAR PRV SWS SWV"
fi
