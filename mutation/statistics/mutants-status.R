...
build_row <- function(operator) {
  ...
  cat(' & ', format(num_killed, scientific=FALSE, big.mark=',', small.mark=','), ' (', sprintf('%.1f', round(per_killed, 2)), '\\%)', sep='')

  num_survived <- nrow(operator_mutants[operator_mutants$'status' == 'survived', ])
  per_survived <- safe_percentage(num_survived, num_mutants)
  cat(' & ', format(num_survived, scientific=FALSE, big.mark=',', small.mark=','), ' (', sprintf('%.1f', round(per_survived, 2)), '\\%)', sep='')

  num_invalid <- nrow(operator_mutants[operator_mutants$'status' == 'invalid', ])
  per_invalid <- safe_percentage(num_invalid, num_mutants)
  cat(' & ', format(num_invalid, scientific=FALSE, big.mark=',', small.mark=','), ' (', sprintf('%.1f', round(per_invalid, 2)), '\\%)', sep='')

  num_timeout <- nrow(operator_mutants[operator_mutants$'status' == 'timeout', ])
  per_timeout <- safe_percentage(num_timeout, num_mutants)
  cat(' & ', format(num_timeout, scientific=FALSE, big.mark=',', small.mark=','), ' (', sprintf('%.1f', round(per_timeout, 2)), '\\%)', sep='')
...
p <- p + annotate('text', x=Inf, y=0, hjust=0.18, vjust=1,
           label=paste0(#
             'Median = ', sprintf('%.1f', round(median_score, 2)), '%', '\n'),
           size=4.5, color='black')
p <- p + annotate('text', x=Inf, y=0, hjust=0.2, vjust=1.6,
           label=paste0(#
            'Mean = ', sprintf('%.1f', round(mean_score, 2)), '%', '\n'),
           size=4.5, color='black')
p <- p + annotate('text', x=Inf, y=0, hjust=0.11, vjust=4.7,
           label=paste0(#
             'Standard Deviation = ', sprintf('%.1f', round(standard_deviation, 2)), '%'),
           size=4.5, color='black')
# Print plot
print(p)
...
p <- p + annotate('text', x=0, y=mean_score, vjust=-0.5, hjust=0,
           label=paste0('Mean = ', sprintf('%.1f', round(mean_score, 2))),
           size=4, color='brown')
...
p <- p + annotate('text', x=0, y=median_score, vjust=-0.5, hjust=0,
           label=paste0('Median = ', sprintf('%.1f', round(median_score, 2))),
           size=4, color='blue')
...
p <- p + annotate('text', x=8, y=max_score, vjust=-0.5, hjust=0,
           label=paste0('Max = ', sprintf('%.1f', round(max_score, 2))),
           size=4, color='red')
# Set labs
